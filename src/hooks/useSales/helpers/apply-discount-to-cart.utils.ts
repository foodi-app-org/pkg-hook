...
export const applyDiscountToCart = (
  ...
  const cartTotalCents = normalizedItems.reduce((s: number, it: any) => s + it.itemTotalCents, 0)
  ...
  const newProducts = normalizedItems.map((it: any, idx: number) => {
    ...
    if (remainingToTake > 0 && it.extrasTotalCents > 0) {
      ...
      const rawExtraAllocs = newExtrasCentsList.map((exCents: number) => (exCents / extrasTotal) * remainingToTake)
      const floorExtraAllocs = rawExtraAllocs.map((v: number) => Math.floor(v))
      let usedExtraFloor = floorExtraAllocs.reduce((s: number, v: number) => s + v, 0)
    ...
    const originalExtrasObjects = it.extrasNormalized.map((e: any) => e.originalExtraObject || {})
  ...
  const allocatedSumCents = breakdown.reduce((s, b) => s + toCents(Number(b.discountAmount) || 0), 0)
